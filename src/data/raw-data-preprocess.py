    ...
    def split_sentences_batch(self, texts: List[str]) -> List[List[str]]:
        ...
        try:
            ...
            for i in range(0, len(valid_texts), self.batch_size):
                ...
                batch_sentences = [[sent.text.strip() for sent in doc.sents if sent.text.strip()] for doc in docs]
...
class SimilarSentenceRemover:
    ...
    def find_similar_sentences(self, sentences):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Trying with more flexible TF-IDF parameters: {str(e)}")
    ...
    def remove_similar_sentences(self, df):
        ...
        self.logger.info(f"Finding and removing similar sentences with threshold {self.similarity_threshold}...")
    ...
    def remove_similar_sentences_parallel(self, df):
        ...
        self.logger.info(f"Finding and removing similar sentences in parallel with {self.n_jobs} processes...")
        original_count = len(df)
        
        # Split data into larger chunks for parallel processing
        parallel_chunk_size = max(self.chunk_size, len(df) // self.n_jobs)
        chunks = [df.iloc[i:i+parallel_chunk_size].copy() for i in range(0, len(df), parallel_chunk_size)]
        ...
        if len(chunks) > 1:
            self.logger.info("Performing final pass to catch cross-boundary similarities...")
...
class DataQualityCheck:
    ...
    def check_duplicates(self) -> Dict:
        ...
        duplicate_pct = (duplicate_count / len(self.df)) * 100 if len(self.df) > 0 else 0
        ...
        for col in self.df.columns:
            if self.df[col].dtype == 'object':  # Only check object/string columns
                ...
                if dup_count > 0:
                    ...
                    column_duplicates[col] = {"count": int(dup_count), "percentage": float(f"{dup_pct:.2f}")}
                    logger.info(f"- Column '{col}' has {dup_count} duplicate values ({dup_pct:.2f}%)")
    ...
    def check_text_quality(self, text_column: str) -> Dict:
        ...
        short_count = ((self.df['text_length'] > 0) & (self.df['text_length'] < 10)).sum()
        ...
        logger.info(f"- Length statistics: min={length_stats['min']}, max={length_stats['max']}, avg={length_stats['mean']:.2f}")
...
class JobDataProcessor:
    ...
    def __init__(self, raw_data_path: Union[str, Path], interim_data_path: Union[str, Path], config: Optional[Dict] = None):
        """
    ...
    def process(self) -> pd.DataFrame:
        ...
        with tqdm(total=len(df_full)) as pbar:
            if self.config['parallelize']:
                with mp.Pool(processes=mp.cpu_count()) as pool:
                    df_full["Topic"] = pool.map(self.cleaner.filter_non_thai, df_full["Topic"].tolist())
                    pbar.update(len(df_full) // 2)
                    df_full["Qualification"] = pool.map(self.cleaner.filter_non_thai, df_full["Qualification"].tolist())
                    pbar.update(len(df_full) // 2)
            else:
                df_full["Topic"] = df_full["Topic"].progress_apply(self.cleaner.filter_non_thai)
                df_full["Qualification"] = df_full["Qualification"].progress_apply(self.cleaner.filter_non_thai)
        ...
        if self.config['parallelize']:
            with mp.Pool(processes=mp.cpu_count()) as pool:
                df_full["Qualification"] = pool.map(self.cleaner.clean_text, df_full["Qualification"].tolist())
        else:
            df_full["Qualification"] = df_full["Qualification"].progress_apply(self.cleaner.clean_text)
        ...
        if self.config['parallelize']:
            ...
            df_full["Segmented_Qualification"] = self.segmenter.split_sentences_batch(df_full["Qualification"].tolist())
        else:
            df_full["Segmented_Qualification"] = df_full["Qualification"].progress_apply(self.segmenter.split_sentences)
        ...
        df = df[df["Segmented_Qualification"].str.len() >= self.config['min_sentence_length']]
        ...
        logger.info(f"Removed {before_dedup - len(df)} duplicate segmented qualifications")
        ...
        quality_stats = data_quality.run_checks(text_columns=["Segmented_Qualification"])
        
        # Save quality report
        quality_report_path = self.interim_data_path / self.config['quality_report_file']
